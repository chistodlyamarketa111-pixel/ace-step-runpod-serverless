...
HF_LORA_REGISTRY = {
    "russian-pop-v1": {
        "repo": "ruslanmusinrusmus/russian-pop-v1",
...
def download_lora_from_hf(lora_name):
    """Download LoRA from HuggingFace and patch target_modules/weight keys."""
    import urllib.request
    import json as json_mod

    if lora_name not in HF_LORA_REGISTRY:
        return False, f"LoRA '{lora_name}' not in HF registry. Known: {list(HF_LORA_REGISTRY.keys())}"

    info = HF_LORA_REGISTRY[lora_name]
    repo = info["repo"]
    lora_dir = os.path.join(LORA_DIR, lora_name)
    ...
    try:
        config_url = f"https://huggingface.co/{repo}/resolve/main/{info['config']}"
        weights_url = f"https://huggingface.co/{repo}/resolve/main/{info['weights']}"

        print(f"[ACE-Step] Downloading LoRA '{lora_name}' from HuggingFace...", flush=True)
        print(f"[ACE-Step]   Config: {config_url}", flush=True)
        urllib.request.urlretrieve(config_url, config_path)
        print(f"[ACE-Step]   Weights: {weights_url}", flush=True)
        urllib.request.urlretrieve(weights_url, weights_path)

        config_size = os.path.getsize(config_path)
        weights_size = os.path.getsize(weights_path)
        print(f"[ACE-Step]   Config: {config_size} bytes, Weights: {weights_size} bytes", flush=True)

        if config_size < 100 or weights_size < 1000:
            return False, f"Download failed: config={config_size}B, weights={weights_size}B (too small)"
        ...
        if new_targets != old_targets:
            print(f"[ACE-Step]   Patching target_modules: {old_targets} -> {new_targets}", flush=True)
        ...
        try:
            ...
            if changes > 0:
                print(f"[ACE-Step]   Renamed {changes} weight keys", flush=True)
                save_file(renamed, weights_path)
        except ImportError:
            print("[ACE-Step]   WARNING: safetensors not available, skipping key rename", flush=True)

        print(f"[ACE-Step]   LoRA '{lora_name}' downloaded and patched OK", flush=True)
        return True, None
    except Exception as e:
        msg = f"Failed to download LoRA '{lora_name}': {e}"
...
def apply_lora(lora_name, lora_scale=1.0):
    ...
    if lora_name not in available:
        print(f"[ACE-Step] LoRA '{lora_name}' not found locally, trying HuggingFace download...", flush=True)
        ok, err = download_lora_from_hf(lora_name)
        if ok:
            available = scan_available_loras()
        if lora_name not in available:
            msg = f"LoRA not found: {lora_name}. Available: {list(available.keys())}"
            if err:
                msg += f". Download error: {err}"
            print(f"[ACE-Step] {msg}", flush=True)
            return False, msg

...
## MACAN LoRA v5 (ACE-Step v1.5 SFT) Training & Inference

### Training (COMPLETED)
- 2000 steps, LoRA rank=64 alpha=128, 11 Russian rap tracks
- Final loss: 0.0007-0.0009, 168MB adapter at `/workspace/lora_output_v15/final/`
- Pod: `jbcczvx9mxxhk8` (A40 46GB)

### Critical Inference Fixes Found
1. **LoRA key mismatch**: Adapter weights have `decoder.` prefix in keys (e.g. `base_model.model.decoder.layers.0...`).
   When applying to `model.decoder`, must remove `decoder.` prefix from both weight keys AND target_modules.
   Fixed adapter saved at `/workspace/lora_fixed/` on RunPod.
2. **src_latents must be silence_latent** (not zeros) for text2music generation
3. **LoRA scale**: Original alpha=128 r=64 (scale=2.0) causes mode collapse (latent std drops 16x).
   Need lower effective scale. Higher alpha values (256-512) on decoder may be needed.

### Inference Pipeline
- Model: `/workspace/acestep-v15-sft/model.safetensors`
- VAE: `/workspace/ace-step-v15-checkpoint/vae/`
- Text encoder: `/workspace/ace-step-v15-checkpoint/Qwen3-Embedding-0.6B/`
- Silence latent: `/workspace/acestep-v15-sft/silence_latent.pt` (transpose(1,2) on load)
- Key files: `macan_lora_v15_final/inference_fixed.py`, `macan_lora_v15_final/inference_merged.py`

### Spectral Analysis Results
- Base model generates REAL music (periodic autocorrelation, harmonic peaks at 101/202/382Hz)
- LoRA effect confirmed but small — need to tune alpha scaling

...
const LORA_OPTIONS = [
  ...
  { id: "russianpop", name: "Русский Поп", description: "190 треков, 84+ артиста" },
  { id: "russianrap", name: "Русский Рэп", description: "150 треков, 10+ артистов" },
  { id: "russiandisco", name: "Русское Диско", description: "Russian disco style" },
  { id: "russianshanson", name: "Русский Шансон", description: "Russian chanson style" },
  { id: "russianrock", name: "Русский Рок", description: "Russian rock style" },
  { id: "russianclassical", name: "Русская Классика", description: "Russian classical music style" },
  { id: "russianelectro", name: "Русское Электро", description: "Russian electronic music style" },
  { id: "russianjazz", name: "Русский Джаз", description: "Russian jazz style" },
  { id: "russianfolk", name: "Русская Народная", description: "Russian folk music style" },
  { id: "russianacoustic", name: "Русская Акустика", description: "Russian acoustic/bard song style" },
  { id: "russianphonk", name: "Русский Фонк", description: "Russian phonk style" },
  { id: "russianrnb", name: "Русский R&B", description: "Russian R&B style" },
  { id: "russianballad", name: "Русская Баллада", description: "Russian ballad style" },
];

const LORA_CHECKPOINTS: Record<string, { id: string; name: string; loss: string }[]> = {
  russianrap: [
    { id: "main", name: "Final (epoch 60)", loss: "1.3437" },
    { id: "epoch_50", name: "Epoch 50", loss: "1.3521" },
    { id: "epoch_40", name: "Epoch 40", loss: "1.3674" },
    { id: "epoch_30", name: "Epoch 30", loss: "1.3882" },
    { id: "epoch_20", name: "Epoch 20", loss: "1.3639" },
    { id: "epoch_10", name: "Epoch 10", loss: "1.4032" },
  ],
};
...
const playgroundSchema = z.object({
  ...
  lora_name: z.string().default("none"),
  lora_scale: z.number().min(0).max(2).default(1.0),
  lora_revision: z.string().default("main"),
...
function PlaygroundTab() {
  ...
  const mutation = useMutation({
    mutationFn: async (data: PlaygroundFormValues) => {
      ...
      if (data.lora_name && data.lora_name !== "none") {
        body.lora_name = data.lora_name;
        body.lora_scale = data.lora_scale;
        if (data.lora_revision && data.lora_revision !== "main") {
          body.lora_revision = data.lora_revision;
        }
      }
      if (data.mastering !== undefined) body.mastering = data.mastering;
      if (data.enhance !== undefined) body.enhance = data.enhance;
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-5">
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel>LoRA Adapter</FormLabel>
                  <Select onValueChange={(val) => { field.onChange(val); form.setValue("lora_revision", "main"); }} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger>
              ...
              render={({ field }) => (
                <FormItem>
                  <div className="flex items-center justify-between">
                    <FormLabel>LoRA Scale</FormLabel>
                  ...
                  <FormControl>
                    <Slider
                      ...
                      max={2}
                      step={0.1}
                      disabled={form.watch("lora_name") === "none"}
                    />
                  </FormControl>
          ...
          {LORA_CHECKPOINTS[form.watch("lora_name")] && (
            <FormField
              control={form.control}
              name="lora_revision"
              render={({ field }) => (
                <FormItem>
                  ...
                  <Select onValueChange={field.onChange} value={field.value}>
                    ...
                    <SelectContent>
                      {LORA_CHECKPOINTS[form.watch("lora_name")]!.map(cp => (
                        <SelectItem key={cp.id} value={cp.id}>
                          <div className="flex items-center gap-2">
                            <span>{cp.name}</span>
                            <span className="text-xs text-muted-foreground">(loss: {cp.loss})</span>
                          </div>
                        </SelectItem>
                      ))}
                    </SelectContent>
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex items-center justify-between gap-4">
                          <div>
                            <FormLabel>Mastering</FormLabel>
                            <p className="text-xs text-muted-foreground">EQ, compression, limiter, LUFS normalization (-14 LUFS)</p>
                          </div>
                          <FormControl>
                            <Switch
                              checked={field.value}
                              onCheckedChange={field.onChange}
                              data-testid="switch-mastering"
                            />
                          </FormControl>
                        </FormItem>
                      )}
                    ...
                    <FormField
                      ...
                      render={({ field }) => (
                        <FormItem className="flex items-center justify-between gap-4">
                          <div>
                            <FormLabel>Audio Enhancement</FormLabel>
                            <p className="text-xs text-muted-foreground">AI denoising and clarity improvement (Resemble Enhance)</p>
                          </div>
                          <FormControl>
                            <Switch
                              checked={field.value}
